...
class DLC_Coest_Recon(object):
    ...
    def recon(self, niter=1000, nsearch_locus=1000, nsearch_coal=1000):
        ...
        for i in xrange(niter):
            ...
            locus_search = lambda ltree: DLCLocusTreeSearch(coal_tree, self.stree, self.gene2species,
            ...
            # update the reconciliation and stage
            coal_tree = maxrecon.coal_tree
            locus_tree = maxrecon.locus_tree
            daughters = maxrecon.daughters
            ...
            coal_search = lambda ctree: DLCCoalTreeSearch(coal_tree, locus_tree, daughters, self.popsizes,
                                                          nprescreen=self.nprescreen_coal)

            self.proposer = DLC_ReconProposer_CoalTree(
                self.stree, locus_tree, daughters,
                self.gene2species,
                search=coal_search)
    ...
    def init_search(self):
        """Initialize new search by setting things to default"""
        # set initial coal tree
        if self.stage == "coal_tree":   
            self.proposer.set_coal_tree(self.init_coal_tree.copy())

        # set initial locus tree
        else:    
            self.proposer.set_locus_tree(self.init_locus_tree.copy())

        # set maximum probability -inf and maxrecon None
        self.maxp = -util.INF
        self.maxrecon = None
    ...
    def eval_proposal(self, proposal):
        ...
        if self.stage == "coal_tree":
            p = reconprob.prob_gene_species_alignment_recon(self.alnfile, self.partfile, self.stree, 
                                                            self.popsizes, self.duprate, self.lossrate, self.subrate,
                                                            self.pretime, self.premean,
                                                            proposal.coal_tree, proposal.coal_recon, self.nsamples_coal,
                                                            proposal.locus_tree, proposal.locus_recon, self.nsamples_locus, proposal.daughters,
                                                            self.threads, self.seed, self.eps,
                                                            info=info)
            util.log("p = %f [maxp = %f]" % (p, self.maxp))
        # estimate locus_tree
        else:
